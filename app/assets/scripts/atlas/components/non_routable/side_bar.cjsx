Comp.SideBar = React.createClass
	...
	getDefaultProps: ->
		props = 
			buttons: [
				{ title: 'Explore Atlas', method: 'projects', icon: 'grid', isToggleable: false }
				{ title: 'Collapse/Expand', method: 'collapse', icon: 'contract', isToggleable: false }
				{ title: 'Help', method: 'help', icon: 'help', isToggleable: false }
				{ title: 'Print', method: 'print', icon: 'print', isToggleable: false }
				{ title: 'Download Data', method: 'download', icon: 'download', isToggleable: false }
			]
		return props
	...
	_renderButtonContent: (options, i) ->
		...
		if options.method is 'download'
			...
			return <form className={ "form--compact bg-img-" + options.icon + "--off-white" } action='/api/v1/projects/print' method='post'> 
				<input type="hidden" name="atlas_url" value={ atlas_url } />
				<input type="submit" value="" />
			</form>
		# regular case
		else if options.method is 'comment'
			return <a href='mailto:atlas@newamerica.org' className={ "bg-img-" + options.icon + "--off-white" }></a>
		else
			return <div className={ "bg-img-" + options.icon + "--off-white" }></div>
